    ...
    def forward(
        ...
        for i, (resnet, attn) in enumerate(blocks):
            ...
            if down_block_add_samples is not None:
                down_block_add_sample = down_block_add_samples.pop(0)
                hidden_states = adaptive_instance_normalization(content_feat=hidden_states, style_feat=down_block_add_sample)
        ...
        if self.downsamplers is not None:
            ...
            if down_block_add_samples is not None:
                down_block_add_sample = down_block_add_samples.pop(0)
                hidden_states = adaptive_instance_normalization(content_feat=hidden_states, style_feat=down_block_add_sample) # todo: add before or after
    ...
    def forward(
        ...
        for resnet, attn in zip(self.resnets, self.attentions):
            ...
            if up_block_add_samples is not None:
                up_block_add_sample = up_block_add_samples.pop(0)
                hidden_states = adaptive_instance_normalization(content_feat=hidden_states, style_feat=up_block_add_sample)  # todo: add before or after
